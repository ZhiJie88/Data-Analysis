...
selected_x_column = ""  # Store the selected x-column
...
def set_x_column(event):
    global selected_x_column
    widget = event.widget
    selected_x_column = widget.get(widget.curselection()).strip()
    ...
    try:
        ...
        for file_path in selected_files:
            try:
                ...
                df.dropna(subset=[selected_x_column] + selected_y_columns, inplace=True)  # Remove rows with missing values in selected columns

                if x_min is None:
                    x_min = df[selected_x_column].min()
                    x_max = df[selected_x_column].max()
                else:
                    x_min = min(x_min, df[selected_x_column].min())
        ...
        plt.title(f'Data from CSV Files: {selected_x_column} vs {", ".join(selected_y_columns)}')
        plt.legend()
...
label_x_columns = tk.Label(window, text="X Column:")
label_x_columns.grid(row=3, column=0, padx=5, pady=5, sticky="w")

# Create the "X Columns" listbox
listbox_x_columns = tk.Listbox(window, selectmode=tk.SINGLE, exportselection=0)
listbox_x_columns.grid(row=4, column=0, padx=5, pady=5)
listbox_x_columns.bind("<<ListboxSelect>>", set_x_column)
